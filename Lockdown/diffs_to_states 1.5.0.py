...
def apply_diff(new_state_row, new_diff_row):
	'''
	@purpose: creates a new state row, using a new value from the diff table.
	'''
...
def add_country_row(old_state_row, new_diff_row):
	'''
	@purpose: creates a new state row for a country,
		taking the previous country state and applying the new change from the diff table
	'''

	new_state_row = old_state_row.copy()

	# takes the seconds value from the previous country state, and adds 1 to it, so the order is kept.
...
def inherit_stringent_rules(province_row, country_row):
	# compare old country state with old province state to get the stringent results
...
def add_province_from_country(country_name, country_row, diff_row):
	'''
	@purpose: inherit more stringent rules for each province in country
	'''
...
def add_province_row(old_country_row, old_province_row, new_diff_row):
	'''
	@purpose: creates a new state row. first, it combines the previous country and the previous province state.
		the method of combining the two is for each field, taking the more stringent value, aka the most severe directive.
		then, it applies the change from the diff table
	'''
...
def process_diff_row(diff_row):
	...
	@note: there are two types of rows considered here: country and province.
		for country rows we simply take the previous row and update it to create the new state.
		for province rows we take the previous province row and the previous row of the matching country.
		we combine those two, picking the more stringent values, then updating the state.
	'''
	...
	if not old_country_row:
		raise ValueError('Earlier state for the country: "{0}" was not found.'.format(country_name))
...
def add_default_states(explored_areas_path):
	'''
	@purpose: add the initial states for all countries and provinces to the states table (1/1/2019)
	'''
...
def diffs_to_states(diff_tables_dir_path, explored_areas_path):
	...
	@purpose: the base function.
		...
		and creates the rows of the states table accordingly.
	'''
